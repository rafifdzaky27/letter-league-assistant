...
export default function TopNav() {
  ...
  const { theme, toggleTheme, gameMode, toggleGameMode } = useAppStore();
    ...
    <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-border-default bg-bg-primary px-8 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
      ...
      <div className="hidden items-center gap-2 md:flex">
        ...
        <button
          ...
          className={`rounded-sm border px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.88px] transition-colors ${
            gameMode === "wild"
              ? "border-accent-yellow-border bg-accent-yellow-bg text-accent-yellow"
              : "border-[rgba(190,194,255,0.3)] bg-accent-indigo-bg text-accent-indigo-light"
          }`}
        >
          {gameMode === "wild" ? "Wild Mode" : "Classic"}
        </button>
        ...
        >
          {theme === "dark" ? (
            ...
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5">
              <path d="M17.5 10.2a7.5 7.5 0 1 1-7.7-7.7 5.5 5.5 0 0 0 7.7 7.7z" />
            </svg>
          )}
        </button>
